...
def _plot_mfs(var_name, fv, x):
    ...
    plt.savefig('Membership{}.png'.format(var_name))
    plt.show()
...
def calc_error(y_pred, y_actual):
    ...
    with torch.no_grad():
        ...
        # print(y_pred.size(), y_actual.size())
        rmse = torch.sqrt(tot_loss).item()
        mae = torch.mean(torch.abs((y_pred - y_actual))).item()
        medae = torch.median(torch.abs((y_pred - y_actual))).item()
...
def train_anfis_with(model, data, optimizer, criterion,
                     epochs=500, show_plots=False, cluster=0, device="cuda:0"):
    '''
    ...
    for t in range(epochs):
        ...
        for x, y_actual in data:
            # y_actual = y_actual.unsqueeze(dim=1)
        ...
        y_pred = model(x)
        mse, rmse, mae = calc_error(y_pred, y_actual)
        # errors.append(loss)
        # Print some progress information as the net is trained:
        if epochs < 30 or t % 10 == 0:
            print('epoch {:4d}: MSE={:.5f}, RMSE={:.5f}, MAE={:.5f}'
                  .format(t, mse, rmse, mae))
            if(errors):
                if (loss <= min(errors)):
                    folder_path = './ANFIS' + str(x.size(1))
                    if not os.path.exists(folder_path):
                        os.mkdir(folder_path)
                    torch.save(model, folder_path + '/ANFIS{}.pth'.format(int(cluster)))
...
def train_anfis(model, data, epochs=500, show_plots=False, cluster=0, device="cuda:0"):
    '''
        Train the given model using the given (x,y) data.
    '''
    optimizer = torch.optim.SGD(model.parameters(), lr=1e-4, momentum=0.99)
    criterion = torch.nn.MSELoss(reduction='sum')
    train_anfis_with(model, data, optimizer, criterion, epochs, show_plots, cluster, device)
