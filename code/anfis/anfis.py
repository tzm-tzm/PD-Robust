...
class AntecedentLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        ants = torch.gather(x.transpose(1, 2), 1, batch_indices.to(x.device))
...
class ConsequentLayer(torch.nn.Module):
    ...
    def fit_coeff(self, x, weights, y_actual):
        ...
        weighted_x_2d = weighted_x.reshape(weighted_x.shape[0], -1)
        y_actual_2d = y_actual.reshape(y_actual.shape[0], -1)
    ...
    def forward(self, x):
        ...
        y_pred = torch.matmul(self.coeff.to(x_plus.device), x_plus.float().t())
...
class AnfisNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        y_pred = torch.bmm(self.rule_tsk, self.weights.float().unsqueeze(2))
