...
class GaussMembFunc(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        val = torch.exp(-torch.pow(x - self.mu.to(x.device), 2) / (2 * self.sigma.to(x.device)**2))
        return val
...
class BellMembFunc(torch.nn.Module):
    ...
    def forward(self, x):
        dist = torch.pow((x - self.c.to(x.device))/self.a.to(x.device), 2)
        return torch.reciprocal(1 + torch.pow(dist, self.b.to(dist.device)))
...
class TriangularMembFunc(torch.nn.Module):
    ...
    def forward(self, x):
        return torch.where(
            (self.a.to(x.device) < x) & (x <= self.b.to(x.device)),
            (x - self.a.to(x.device)) / (self.b.to(x.device) - self.a.to(x.device)),
            # else
            torch.where(
                (self.b.to(x.device) < x) & (x <= self.c.to(x.device)),
                (self.c.to(x.device) - x) / (self.c.to(x.device) - self.b.to(x.device)),
